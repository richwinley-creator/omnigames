...
export default function MeterImport({ onSave }) {
  ...
  const processImage = useCallback(async (file) => {
    ...
    reader.onload = async () => {
      ...
      setExtracting(true);
      try {
        const result = await apiPost('/api/extract', { image: base64, mimeType });
        setExtracted(result);

        // Try to auto-match location
  ...
  const updateMachine = (idx, field, value) => {
    setExtracted(prev => ({
      ...prev,
      machines: prev.machines.map((m, i) => i === idx ? { ...m, [field]: parseFloat(value) || 0 } : m),
    }));
  };
  ...
  return (
    <div style={styles.section} onPaste={onPaste} tabIndex={0}>
      ...
      <p style={styles.subtitle}>Drop a photo, paste from clipboard, or click to upload. AI extracts the numbers automatically.</p>
      ...
      {extracted && (
        <div>
              ...
              <tbody>
                {extracted.machines.map((m, i) => {
                  ...
                  return (
                    <tr key={i}>
                      <td style={styles.td}>
                        <input style={styles.input} value={m.machine_name} onChange={e => updateMachine(i, 'machine_name', e.target.value)} />
                      </td>
                      <td style={styles.td}>
                        <input style={styles.input} type="number" value={m.prev_in} onChange={e => updateMachine(i, 'prev_in', e.target.value)} />
                      </td>
                      <td style={styles.td}>
                        <input style={styles.input} type="number" value={m.curr_in} onChange={e => updateMachine(i, 'curr_in', e.target.value)} />
                      </td>
                      <td style={styles.td}>
                        <input style={styles.input} type="number" value={m.prev_out} onChange={e => updateMachine(i, 'prev_out', e.target.value)} />
                      </td>
                      <td style={styles.td}>
                        <input style={styles.input} type="number" value={m.curr_out} onChange={e => updateMachine(i, 'curr_out', e.target.value)} />
                      </td>
          ...
          {selectedLocation && (() => {
            ...
            if (!loc) return null;
            const totalNet = extracted.machines.reduce((s, m) => {
              const ti = (m.curr_in || 0) - (m.prev_in || 0);
              const to = (m.curr_out || 0) - (m.prev_out || 0);
              return s + ti - to;
            }, 0);
            return (
